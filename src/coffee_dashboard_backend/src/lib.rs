...
type SensorDataStore = BTreeMap<String,Vec<SensorData>>; 
...
thread_local! {
    ...
    static SENSOR_DATA_STORE: RefCell<SensorDataStore> = RefCell::default();
}
...
#[update]
fn add_farm(id: String, name: String, metadata: String) {
    let geohash: String;
    ...
    match get_farm(name.clone()){
        ...
        None => {
            ...
            FARMS_STORE.with(|farms_store| {
                ...
                geo_index::index(geohash.to_string(),id);
            });
        }
    } 
}
...
fn add_data(id: String, ts: i64, farm: String, metadata: String) {
    //let geohash: String;
    let geohash = geohash_from_id(id.to_string());
    geo_index::index(geohash.to_string(),id.to_string());
...
#[update]
fn add_sensor_data(id: String, data: String, ts: i64) {
    geoindex_id(id.to_string());
    let new_data = SensorData {ts, data };
    SENSOR_DATA_STORE.with(|data_store| {
        let mut data_store_mut = data_store.borrow_mut();
        match data_store_mut.entry(id.clone()) {
            Entry::Vacant(entry) => {
                entry.insert(vec![new_data]);
            }
            Entry::Occupied(mut entry) => {
                entry.get_mut().push(new_data);
            }
        }        
    });
}
...
#[query]
fn get_sensor_data(id: String, ts_from: i64) -> Vec<SensorData> {
    SENSOR_DATA_STORE.with(|data_store| {
        match data_store.borrow().get(&id){
            Some(data) => {
                data.iter()
                .filter(|&data| data.ts>=ts_from).cloned().collect()
            }
            None => {
                let empty: Vec<SensorData> = Vec::new();
                empty
            }
        }
...
#[query]
fn geo_index_view() -> Vec<String>{
    geo_index::view_index()
}

#[query]
fn geohash_from_id(id: String) -> String{
    match id_tools::geohash_from_id(&id){
        Ok(geohash) =>{
            geohash
        }
        Err(_) => {
...
#[update]
fn geoindex(geohash: String, id: String) {
...
#[update]
fn geoindex_id(id: String) -> String {
    
    let geohash = geohash_from_id(id.clone());
    geo_index::index(geohash.clone(),id.clone());
    geohash_from_id(id.clone())
    
}

#[query]
fn geo_lookup(id: String) -> String{
    geo_index::lookup(&id)
}

#[query]
fn get_distance(geohash1: String, geohash2: String) -> f64{
    geo_index::get_distance_from_geohash(geohash1, geohash2)
}

#[update]
fn upload_image(blob: Vec<u8>) -> String {
    image::upload(blob)
}

#[query]
fn get_image(file_id: String) -> Vec<u8>{
    image::get(file_id)
}
